...
function ot_joint(
    ...
    @constraint(
        modelA,
        ctYandXinA[x in 1:nbX, y in Y],
        sum(gammaA[x, y, z] for z in Z) == estim_XA_YA[x, y] + errorA_XY[x, y]
    )
    @constraint(
        modelB,
        ctZandXinB[x in 1:nbX, z in Z],
        sum(gammaB[x, y, z] for y in Y) == estim_XB_ZB[x, z] + errorB_XZ[x, z]
    )
    ...
    @constraint(
        modelA,
        ctZandXinA[x in 1:nbX, z in Z],
        estim_XB[x] * sum(gammaA[x, y, z] for y in Y) ==
        estim_XB_ZB[x, z] * estim_XA[x] + estim_XB[x] * errorA_XZ[x, z]
    )

    @constraint(
        modelB,
        ctYandXinB[x in 1:nbX, y in Y],
        estim_XA[x] * sum(gammaB[x, y, z] for z in Z) ==
    ...
    @constraint(modelA, [x in 1:nbX, y in Y], -errorA_XY[x, y] <= abserrorA_XY[x, y])
    @constraint(modelA, sum(abserrorA_XY[x, y] for x = 1:nbX, y in Y) <= maxrelax / 2.0)
    @constraint(modelA, sum(errorA_XY[x, y] for x = 1:nbX, y in Y) == 0.0)
    @constraint(modelA, [x in 1:nbX, z in Z], errorA_XZ[x, z] <= abserrorA_XZ[x, z])
    @constraint(modelA, [x in 1:nbX, z in Z], -errorA_XZ[x, z] <= abserrorA_XZ[x, z])
    @constraint(modelA, sum(abserrorA_XZ[x, z] for x = 1:nbX, z in Z) <= maxrelax / 2.0)
    @constraint(modelA, sum(errorA_XZ[x, z] for x = 1:nbX, z in Z) == 0.0)

    @constraint(modelB, [x in 1:nbX, y in Y], errorB_XY[x, y] <= abserrorB_XY[x, y])
    @constraint(modelB, [x in 1:nbX, y in Y], -errorB_XY[x, y] <= abserrorB_XY[x, y])
    @constraint(modelB, sum(abserrorB_XY[x, y] for x = 1:nbX, y in Y) <= maxrelax / 2.0)
    @constraint(modelB, sum(errorB_XY[x, y] for x = 1:nbX, y in Y) == 0.0)
    @constraint(modelB, [x in 1:nbX, z in Z], errorB_XZ[x, z] <= abserrorB_XZ[x, z])
    @constraint(modelB, [x in 1:nbX, z in Z], -errorB_XZ[x, z] <= abserrorB_XZ[x, z])
    @constraint(modelB, sum(abserrorB_XZ[x, z] for x = 1:nbX, z in Z) <= maxrelax / 2.0)
    @constraint(modelB, sum(errorB_XZ[x, z] for x = 1:nbX, z in Z) == 0.0)

    # - regularization
    @variable(
        modelA,
        reg_absA[x1 in 1:nbX, x2 in findall(voisins_X[x1, :]), y in Y, z in Z] >= 0
    )
    @constraint(
        modelA,
        [x1 in 1:nbX, x2 in findall(voisins_X[x1, :]), y in Y, z in Z],
        reg_absA[x1, x2, y, z] >=
    ...
    @expression(
        ...
        sum(
            ...
            x2 in findall(voisins_X[x1, :]), y in Y, z in Z
        )
    )

    @variable(
        modelB,
        reg_absB[x1 in 1:nbX, x2 in findall(voisins_X[x1, :]), y in Y, z in Z] >= 0
    )
    @constraint(
        modelB,
        [x1 in 1:nbX, x2 in findall(voisins_X[x1, :]), y in Y, z in Z],
    ...
    @objective(
        modelA,
        Min,
        sum(C[y, z] * gammaA[x, y, z] for y in Y, z in Z, x = 1:nbX) +
        lambda_reg * sum(
            1 / length(voisins_X[x1, :]) * reg_absA[x1, x2, y, z] for x1 = 1:nbX,
            x2 in findall(voisins_X[x1, :]), y in Y, z in Z
        )
    ...
    @objective(
        modelB,
        Min,
        sum(C[y, z] * gammaB[x, y, z] for y in Y, z in Z, x = 1:nbX) +
        lambda_reg * sum(
            1 / length(voisins_X[x1, :]) * reg_absB[x1, x2, y, z] for x1 = 1:nbX,
            x2 in findall(voisins_X[x1, :]), y in Y, z in Z
        )
    ...
    gammaA_val = [value(gammaA[x, y, z]) for x = 1:nbX, y in Y, z in Z]
    gammaB_val = [value(gammaB[x, y, z]) for x = 1:nbX, y in Y, z in Z]
    ...
    Solution(
        time() - tstart,
        [sum(gammaA_val[x, y, z] for x = 1:nbX) for y in Y, z in Z],
        [sum(gammaB_val[x, y, z] for x = 1:nbX) for y in Y, z in Z],
        estimatorZA,
        estimatorYB,
    )
