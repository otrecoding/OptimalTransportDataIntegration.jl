...
function generate_XY(params, binsA1, binsA2, binsA3, binsB1, binsB2, binsB3, r2)
    ...
    binsA1 = vcat(minimum(XA[1, :]) - 100, qxA1c, maximum(X_glob1[1, :]) + 100)
    binsA2 = vcat(minimum(XA[2, :]) - 100, qxA2c, maximum(X_glob1[2, :]) + 100)
    binsA3 = vcat(minimum(XA[3, :]) - 100, qxA3c, maximum(X_glob1[3, :]) + 100)

    binsB1 = vcat(minimum(XB[1, :]) - 100, qxB1c, maximum(X_glob2[1, :]) + 100)
    binsB2 = vcat(minimum(XB[2, :]) - 100, qxB2c, maximum(X_glob2[2, :]) + 100)
    binsB3 = vcat(minimum(XB[3, :]) - 100, qxB3c, maximum(X_glob2[3, :]) + 100)
...
"""
function generate_data(params::DataParameters, r2::Float64)
    ...
    X1, X2, X3, Y1, Y2 = generate_XY(params, binsA1, binsA2, binsA3, binsB1, binsB2, binsB3, r2)
