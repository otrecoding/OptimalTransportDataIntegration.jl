...
julia> round(entropic_partial_wasserstein(a, b, M, 1, 0.1), digits=2)
2×2 Matrix{Float64}:
 0.06  0.02
 0.01  0.0
```
...
function entropic_partial_wasserstein(a, b, M, reg, m=nothing, numItermax=1000, stopThr=1e-100, verbose=false, log=false)
    ...
    while err > stopThr && cpt < numItermax
        Kprev .= K
        K .= K .* q1
        K1 .= Diagonal(vec(min.(a ./ sum(K, dims=2), dx))) * K
        q1 .= q1 .* Kprev ./ K1
        K1prev .= K1
        K1 .= K1 .* q2
        K2 .= K1 * Diagonal(vec(min.(b ./ sum(K1, dims=2), dy)))
        q2 .= q2 .* K1prev ./ K2
        K2prev .= K2
        K2 .= K2 .* q3
        K .= K2 .* (m ./ sum(K2))
...
a = [.1, .2, .3, .4]
b = [.1, .1, .1, .1]
M = [0. 1. 3. 4.; 2. 3. 5. 6.; 7. 8. 1. 2.; 3. 4. 5. 6.]
@btime Kjl = round.(entropic_partial_wasserstein($a, $b, $M, 1.0, 0.1), digits=2) 
@btime Kpy = round.(PythonOT.entropic_partial_wasserstein(a, b, M, 1.0, m = 0.1), digits=2)
Kjl ≈ Kpy
